...
class DataProvider(object):
    def __init__(self, config):
        if config.dataset == 'celebA':
            ...
            self.data = glob(os.path.join("./data/", config.dataset, "*.JPEG"))
            # print(glob(os.path.join("./data/", config.dataset, "*.JPEG")))
        ...
        if config.color_space == "YUV":
            for batch_file in batch_files:
                batch_images = np.array(
                    [(cvtRGB2YUV(misc.imread(batch_file)) / 127.5 - 1.) for batch_file in batch_files],
                    dtype=np.float32)
        elif config.color_space == "RGB":
            batch_images = np.array([(misc.imread(batch_file) / 127.5 - 1.) for batch_file in batch_files],
                                    dtype=np.float32)
        else:
...
class WGAN(object):
    ...
    def build_model(self, config):
        ...
        self.images = tf.placeholder(tf.float32, [config.batch_size] + [config.image_size, config.image_size, 3],
                                     name='real_images')
        ...
        if config.improved_wgan:
            ...
            gradients = tf.gradients(self.discriminator_wgan(interpolates, reuse=True, config=config), [interpolates])[
                0]
    ...
    def train(self, config=None):
        ...
        if config.optimizer == "Adam":
            d_optim = tf.train.AdamOptimizer(config.d_learning_rate, beta1=config.adam_beta1).minimize(self.d_loss,
                                                                                                       var_list=self.d_vars)
            g_optim = tf.train.AdamOptimizer(config.g_learning_rate, beta1=config.adam_beta1).minimize(self.g_loss,
                                                                                                       var_list=self.g_vars)
        elif config.optimizer == "RMSProp":
            d_optim = tf.train.RMSPropOptimizer(config.d_learning_rate, decay=0.9).minimize(self.d_loss,
                                                                                            var_list=self.d_vars)
            g_optim = tf.train.RMSPropOptimizer(config.g_learning_rate, decay=0.9).minimize(self.g_loss,
                                                                                            var_list=self.g_vars)
        else:
        ...
        save_images(sample_images[:save_size * save_size], [save_size, save_size],
                    '{}/train_{:02d}_{:05d}.png'.format(config.sample_dir, 0, 0), color_space=config.color_space)
        ...
        while data.epoch_idx < config.epoch:
            ...
            log_txt.write(
                "{:d} {:d} {:d} {:.8f} {:.8f} {:.8f}\n".format(data.epoch_idx, data.batch_idx, data.batch_idxs, _loss,
                                                               _g_loss, _d_loss))

            # 控制保存时间
            if (counter % sample_interval == 1):
                save_size = int(math.sqrt(config.batch_size))  # 8
                for sample_idx in range(config.sample_times):  # 3
                    _generate_image, _g_loss, _d_loss, _loss = self.sess.run(
                        [self.generate_image, self.g_loss, self.d_loss, self.total_loss],
                        feed_dict={self.z: sample_z[sample_idx], self.images: sample_images})
                    save_images(_generate_image[:save_size * save_size], [save_size, save_size],
                                '{}/train_{:02d}_{:05d}_z{:d}.png'.format(config.sample_dir, data.epoch_idx,
                                                                          data.batch_idx, sample_idx + 1),
                                color_space=config.color_space)
                    print("[Sample] loss z%d: %.8f, g_loss: %.8f,\n\t d_loss: %.8f" % (
                    sample_idx + 1, _loss, _g_loss, _d_loss))
    ...
    def discriminator_wgan(self, image, y=None, reuse=False, config=None):
        with tf.variable_scope("discriminator") as scope:
            ...
            h0 = lrelu(conv2d(image, 64, k_h=config.d_kernel_size, k_w=config.d_kernel_size, name='d_h0_conv'),
                       name='d_bn0')
            h1 = lrelu(batch_norm(conv2d(h0, 128, k_h=config.d_kernel_size, k_w=config.d_kernel_size, name='d_h1_conv'),
                                  name='d_bn1'))
            h2 = lrelu(batch_norm(conv2d(h1, 256, k_h=config.d_kernel_size, k_w=config.d_kernel_size, name='d_h2_conv'),
                                  name='d_bn2'))
            h3 = lrelu(batch_norm(conv2d(h2, 512, k_h=config.d_kernel_size, k_w=config.d_kernel_size, name='d_h3_conv'),
                                  name='d_bn3'))
    ...
    def test_fix(self, config=None):
        ...
        save_images(test_image_batch, [save_size, save_size],
                    '{}/test_fixed_origin_{:01d}.png'.format(config.sample_dir, config.test_offset),
                    color_space=config.color_space)
        ...
        with open("test_z_fixed.pkl", 'r') as infile:
            test_z_batches = pickle.loads(infile)
        ...
        for test_round_idx in range(config.batch_size):
            ...
            _generate_image, _g_loss, _d_loss = self.sess.run([self.generate_image, self.g_loss, self.d_loss],
                                                              feed_dict={self.z: test_z_batches[test_round_idx],
                                                                         self.images: test_image_batch})
            print("g_loss: %.8f, d_loss: %.8f" % (_g_loss, _d_loss))
            save_images(_generate_image[:save_size * save_size], [save_size, save_size],
                        '{}/test_fixed_round_{:01d}{:02d}.png'.format(config.sample_dir, config.test_offset,
                                                                      test_round_idx), color_space=config.color_space)
        ...
        with open('{}/test_fixed_prob_{:01d}.pkl'.format(config.sample_dir, config.test_offset), 'w') as outfile:
            cloudpickle.dumps((test_image_idxs, test_images, test_z_batches, save_result_g_loss, save_result_d_loss),
                              outfile)
        print('Save done.')
